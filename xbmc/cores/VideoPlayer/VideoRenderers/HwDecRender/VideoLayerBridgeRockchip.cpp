...
const std::string SETTING_VIDEOSCREEN_HDMIOUTPUTFORMAT = "videoscreen.hdmioutputformat";
const std::string SETTING_VIDEOPLAYER_HDMIOUTPUTFORMAT = "videoplayer.hdmioutputformat";

static int GetOutputFormat(bool videoLayer)
{
  int outputFormat = 0;
  if (videoLayer)
  ...
  if (outputFormat == DRM_OUTPUT_FORMAT_YCBCR)
...
void CVideoLayerBridgeRockchip::Configure(CVideoBufferDRMPRIME* buffer)
{
  ...
  m_DRM->AddProperty(connector, "hdmi_output_format", GetOutputFormat(true));
...
void CVideoLayerBridgeRockchip::Disable()
{
  ...
  m_DRM->AddProperty(connector, "hdmi_output_format", GetOutputFormat(false));
